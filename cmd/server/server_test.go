...
func TestListenAndServe(t *testing.T) {
	...
	err = s.ListenAndServe(ctx, "rpcUrl", "evoRpc", "localhost:8080", stateService)
	if err != nil {
		t.Fatalf("got unexpected error: %v, expected: no error", err)
	}
}
...
func TestListenAndServeWithCancel(t *testing.T) {
	...
	go func() {
		err := s.ListenAndServe(ctx, "rpcUrl", "evoRpc", ":9999", stateService) // using a random port, as it won't actually bind
		done <- err
	}()
...
func TestListenAndServeWithError(t *testing.T) {
	...
	err = s.ListenAndServe(ctx, "rpcUrl", "evoRpc", ":9999", stateService)
	if err == nil {
