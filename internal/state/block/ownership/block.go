...
func (s *service) SetEvoCurrentBlockForOwnershipContract(contract string, blockNumber uint64) error {
	return s.tx.Set([]byte(contractEvoCurrentBlockPrefix+strings.ToLower(contract)), []byte(strconv.FormatUint(blockNumber, 10)))
}

func (s *service) GetEvoCurrentBlockForOwnershipContract(contract string) (uint64, error) {
	defer s.tx.Discard()
