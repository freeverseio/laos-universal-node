...
func TestUpdateState(t *testing.T) {
	...
	tx.EXPECT().GetCollectionAddress("0x01").Return(common.HexToAddress("0x03"), nil).Times(2)
	tx.EXPECT().GetCollectionAddress("0x02").Return(common.HexToAddress("0x04"), nil).Times(1)
	tx.EXPECT().AccountData(common.HexToAddress("0x01")).Return(&account.AccountData{
		LastProcessedEvoBlock: 351,
	}, nil).Times(2)
	tx.EXPECT().GetNextEvoEventBlock(common.HexToAddress("0x03").String(), uint64(351)).Return(uint64(0), nil).Times(2)
	tx.EXPECT().AccountData(common.HexToAddress("0x02")).Return(&account.AccountData{
		LastProcessedEvoBlock: 351,
	}, nil).Times(1)
	tx.EXPECT().GetNextEvoEventBlock(common.HexToAddress("0x04").String(), uint64(351)).Return(uint64(0), nil).Times(1)
	tx.EXPECT().TagRoot(int64(100)).Return(nil).Times(1)
	tx.EXPECT().TagRoot(int64(101)).Return(nil).Times(1)

	client.EXPECT().HeaderByNumber(context.Background(), big.NewInt(100)).Return(&types.Header{
		Time:   10,
		Number: big.NewInt(int64(100)),
	}, nil)
	client.EXPECT().HeaderByNumber(context.Background(), big.NewInt(101)).Return(&types.Header{
		Time:   11,
		Number: big.NewInt(int64(101)),
	}, nil)

	u := uUpdater.New(client, scanner)
	err := u.UpdateState(ctx, tx, contracts, newContracts, modelTransferEvents, startingBlock, lastBlockData)
	if err != nil {
